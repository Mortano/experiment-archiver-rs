...
docker run -d --name exar-postgres-test -p $PORT:5432 -e POSTGRES_PASSWORD=$POSTGRES_PASSWORD -e POSTGRES_DB=$POSTGRES_DB -v ./data:/docker-entrypoint-initdb.d/ postgres:latest
